...
func NewManager(cfg Config, st *store.Store, ts *TrackSource, mediaSigner *tokens.MediaSigner, issuer *tokens.Issuer, log *slog.Logger) *Manager {
	return &Manager{
		games:       map[string]*ManagedGame{},
		byInvite:    map[string]string{},
		cfg:         cfg,
...
func (m *Manager) CreateGame(displayName string, targetCards, startTokens *int, enableSongGuess *bool) (PlayerIdentity, error) {
	...
	mg := newManagedGame(id, code, settings, m.trackSource, m.mediaSigner, m.st, m.cfg, m.log, m)
	m.games[id] = mg
	m.byInvite[code] = id
	m.mu.Unlock()
...
func (m *Manager) RehydrateFromSnapshots(ctx context.Context) {
	...
	for _, snap := range snapshots {
		mg := newManagedGame(snap.GameID, snap.InviteCode, game.Settings{}, m.trackSource, m.mediaSigner, m.st, m.cfg, m.log, m)
