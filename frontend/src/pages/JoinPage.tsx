...
export default function JoinPage() {
  ...
  return (
    <div className="min-h-screen bg-bg flex items-center justify-center px-4">
      <form onSubmit={submit} className="card-surface w-full max-w-sm p-8 flex flex-col gap-4 items-center">
        <h1 className="text-lg font-semibold">{t("join.hostedBy", { name: preview.hostName })}</h1>
        <p className="text-sm text-white/60">{t("join.playerCount", { count: preview.playerCount, max: preview.maxPlayers })}</p>
        <input
          autoFocus
          value={name}
          onChange={(e) => setName(e.target.value)}
          placeholder={t("join.namePlaceholder")}
          className="w-full bg-black/30 border border-border rounded-lg py-2.5 px-3 outline-none focus:border-accent"
          maxLength={20}
        />
        ...
        <button
          type="submit"
          disabled={busy}
          className="w-full bg-accent hover:brightness-110 transition-[filter] text-white font-semibold py-2.5 rounded-lg disabled:opacity-50"
        >
          {t("join.submit")}
        </button>
      </form>
